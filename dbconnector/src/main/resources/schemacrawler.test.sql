...
CREATE TABLE PRODUCT(ID INTEGER NOT NULL PRIMARY KEY,NAME VARCHAR(255),PRICE FLOAT);
CREATE TABLE CUSTOMER(ID INTEGER NOT NULL PRIMARY KEY,FIRSTNAME VARCHAR(255),LASTNAME VARCHAR(255),STREET VARCHAR(255),CITY VARCHAR(255));
CREATE TABLE INVOICE(ID INTEGER NOT NULL PRIMARY KEY,CUSTOMERID INTEGER,TOTAL FLOAT);
CREATE TABLE SUPPLIER(SUPPLIER_ID INTEGER,SUPPLIER_NAME VARCHAR(255));

-- Views
CREATE VIEW CUSTOMERLIST (ID,FIRSTNAME,LASTNAME) AS SELECT ID, FIRSTNAME, LASTNAME FROM CUSTOMER;
...
CREATE INDEX INDEX_ITEM_CUSTOMER ON INVOICE(CUSTOMERID);
CREATE INDEX INDEX_SUPPLIER_B ON SUPPLIER(SUPPLIER_ID, SUPPLIER_NAME);
CREATE INDEX INDEX_SUPPLIER_A ON SUPPLIER(SUPPLIER_NAME, SUPPLIER_ID);

-- Foreign keys
ALTER TABLE ITEM ADD CONSTRAINT FK_ITEM_B FOREIGN KEY(INVOICEID) REFERENCES INVOICE(ID);
ALTER TABLE ITEM ADD CONSTRAINT FK_ITEM_A FOREIGN KEY(PRODUCTID) REFERENCES PRODUCT(ID);
ALTER TABLE INVOICE ADD CONSTRAINT FK_CUSTOMER FOREIGN KEY(CUSTOMERID) REFERENCES CUSTOMER(ID);

-- Check constraints
ALTER TABLE SUPPLIER ADD CONSTRAINT CHECK_UPPERCASE_SUPPLIER_NAME CHECK(SUPPLIER_NAME=UPPER(SUPPLIER_NAME));
