...
public class CompactSerializationTest {
  ...
  @Test
  public void executableSerializeCompactJson(final DatabaseConnectionSource connectionSource)
      throws Exception {
    ...
    final Path outputFile =
        executableExecution(connectionSource, executable, SerializationFormat.compact_json);

    // Compact JSON starts with '{' (0x7B), not indented (no newline before first
    // property)
    assertThat(fileHeaderOf(outputFile), is(oneOf("7B0D", "7B0A", "7B22")));
  }
}
