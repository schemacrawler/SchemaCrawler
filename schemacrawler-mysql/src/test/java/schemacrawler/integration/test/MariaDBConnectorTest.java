...
public class MariaDBConnectorTest {
	...
	public void shouldResolveConnectorForMariaDbJdbcUrl() throws SQLException {
		...
		final boolean hasMySqlConnector = DatabaseConnectorRegistry.getRegistry().hasDatabaseSystemIdentifier("mysql");
		...
		final DatabaseConnectionSource connectionSource = DatabaseConnectionSources.fromConnection(connection);

		// Fails in 17.15.x with:
		// InternalRuntimeException: Add the SchemaCrawler database connector plugin for
		// <mysql> to the CLASSPATH
		assertDoesNotThrow(() -> {
			final SchemaRetrievalOptions options = DatabaseConnectorUtility
